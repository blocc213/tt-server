...
两个宿主都提供完整的管理面。Tauri host 走 `src-tauri/crates/tauritavern/src/presentation/commands/extension_commands.rs`；`tauritavern-server` 在 `src-tauri/crates/tauritavern-server/src/rpc.rs` 的 `EXPOSED_COMMANDS` 与 `run` 中暴露同一组命令（`install_extension` / `update_extension` / `delete_extension` / `get_extension_version` / `get_extension_branches` / `switch_extension_branch` / `move_extension`），共用 `ExtensionService` 与同一个 permit。server 不套 iOS capability gate：它从不在 iOS 分发策略下运行，取无限制基线。新增管理命令必须同时改这两处，否则浏览器端会收到 `Command ... is not available in server mode` 的 404。
