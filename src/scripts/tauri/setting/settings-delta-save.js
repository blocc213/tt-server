...
import { hasBackend } from '../../../tauri-bridge.js';
...
export async function trySaveSettingsDelta(prepared, headers) {
    // Any TauriTavern backend, app or server. Gating on `isTauri()` alone sent
    // the browser down the full-settings path, which has no baseline check:
    // two devices saving from the same baseline silently clobbered each other
    // (`SettingsService::save_user_settings` only compares hashes to skip
    // no-op writes, it never validates what the writer was editing).
    if (!hasBackend()) {
        return { saved: false, reason: 'no-backend' };
    }
