...
// Host detection, before any dynamic import.
//
// The desktop/mobile app injects Tauri APIs into the page; the server host does
// not, and serves the same bundle over plain HTTP instead. Deciding here keeps
// later modules from racing against bridge injection.
//
// `__TAURI_RUNNING__` stays true only under the real app: upstream SillyTavern
// scripts use it to mean "native app capabilities are present" (native share,
// OS dialogs, file pickers). Server mode gets its own flag so those branches
// keep their meaning.
const isNativeAppRuntime = typeof window.__TAURI_INTERNALS__ !== 'undefined'
    || typeof window.__TAURI__?.core?.invoke === 'function';

window.__TAURI_RUNNING__ = isNativeAppRuntime;
window.__TAURITAVERN_SERVER__ = !isNativeAppRuntime;
