...
export async function installBackendErrorBridge() {
    // The app mirrors error-level backend logs into a webview event stream.
    // The server reports failures in each HTTP response instead, so there is no
    // event stream to attach to and nothing queued to drain.
