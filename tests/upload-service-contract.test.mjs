...
function installRuntimeGlobals({ userAgent, platform = '', maxTouchPoints = 0, server = false }) {
    ...
    globalThis.window = server
        ? { __TAURITAVERN_SERVER__: true }
        : {
            __TAURI__: {
                core: {
                    invoke: async () => {
...
test('Android browser upload in server mode uses raw chunks, not the app base64 workaround', async () => {
    ...
    const service = createHostStagingService({
        calls,
        filePath: '/server/.server-upload-staging/avatar/upload.png',
        expectedChunkEncoding: 'raw',
        expectedBeginDto: {
            kind: 'avatar',
            preferred_extension: 'png',
    ...
    try {
        ...
        assert.equal(fileInfo.isTemporary, true);
        const chunkCalls = calls.filter(call => call.command === 'stage_upload_chunk');
        assert.deepEqual(chunkCalls.map(call => call.options.headers.offset), ['0', '4', '8']);
        assert.deepEqual(chunkCalls.map(call => call.options.headers['chunk-encoding']), [undefined, undefined, undefined]);
        assert.deepEqual(chunkCalls.map(call => call.args.byteLength), [4, 4, 2]);
    } finally {
        restore();
    }
});
