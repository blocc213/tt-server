...
/// The desktop/mobile app is single-client, so its saves keep upstream's
/// slug-only semantics: `force` skips the check, anything else relies on the
/// chat integrity slug. Revision preconditions are a server-host concern.
fn precondition_from_force(force: bool) -> ChatPayloadPrecondition {
    if force {
...
pub async fn begin_chat_commit(
    ...
    let session = app_state
        ...
        .begin(target, precondition_from_force(force))
        .await?;
