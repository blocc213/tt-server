...
use tt_ports::repositories::chat_payload_commit_repository::{
    ChatPayloadCommitBegin, ChatPayloadCommitRepository, ChatPayloadPrecondition,
    ChatPayloadTarget, ChatPayloadVersion, CommittedChatPayload,
};
use uuid::Uuid;
...
/// Hashes the stored payload to produce its revision token.
///
/// Always reads the file rather than trusting an in-memory signature: the data
/// root can be edited by other tools, and a stale token here would silently
/// approve an overwrite.
async fn read_payload_version(path: &Path) -> Result<Option<ChatPayloadVersion>, DomainError> {
    let bytes = match fs::read(path).await {
        Ok(bytes) => bytes,
        Err(error) if error.kind() == io::ErrorKind::NotFound => return Ok(None),
        Err(error) => {
            return Err(DomainError::InternalError(format!(
                "Failed to read chat payload {}: {error}",
                path.display()
            )));
        }
    };
...
impl ChatPayloadCommitRepository for FileChatRepository {
    ...
    async fn read_version(
        ...
        let target_path = self.resolve_chat_commit_target(target).await?;
        let _guard = self.acquire_payload_snapshot_lock(&target_path).await;
        read_payload_version(&target_path).await
    }
    ...
    async fn finish(
        ...
        let precondition = session.precondition.clone();
        ...
        let result = async {
            ...
            // Precondition and publish share one lock hold: another save cannot
            // slip between the check and the rename.
            match &precondition {
                ChatPayloadPrecondition::Overwrite => {}
                ChatPayloadPrecondition::IntegritySlug => {}
                ChatPayloadPrecondition::MustNotExist => {
                    if fs::try_exists(&target_path).await.unwrap_or(true) {
                        return Err(DomainError::Conflict(
                            "integrity: chat already exists".to_string(),
                        ));
                    }
                }
                ChatPayloadPrecondition::MatchesVersion(expected) => {
                    match read_payload_version(&target_path).await? {
                        Some(current) if current == *expected => {}
                        _ => {
                            return Err(DomainError::Conflict(
                                "integrity: chat changed on the server".to_string(),
                            ));
                        }
                    }
                }
            ...
            // The upstream slug guards against writing one chat's payload over a
            // different chat's file. An explicit Overwrite skips it, matching
            // upstream's `force` flag.
            if !matches!(precondition, ChatPayloadPrecondition::Overwrite) {
            ...
            let version = read_payload_version(&target_path).await?.ok_or_else(|| {
                DomainError::InternalError(
                    "Published chat payload disappeared before versioning".to_string(),
                )
            })?;
