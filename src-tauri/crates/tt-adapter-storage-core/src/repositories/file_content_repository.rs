...
pub struct FileContentRepository {
    resources: Arc<dyn BundledResourceStore>,
    data_root: PathBuf,
...
impl FileContentRepository {
    ...
    /// # Arguments
    ///
    /// * `resources` - Packaged default-content reader
    ...
    fn expand_resource_entries(&self, filename: &str) -> Result<Vec<String>, DomainError> {
        ...
        let entries = self.resources.list_default_content_files_under(filename);
        if entries.is_empty() {
    ...
    async fn seed_content_scope(
        ...
        for item in scoped_items {
            ...
            for resource_entry in resource_entries {
                ...
                self.copy_resource_to_file(&resource_path, &dest_path).await?;
            }
    ...
    fn read_resource_json<T: serde::de::DeserializeOwned>(
        ...
        let text = self.resources.read_text(relative_path)?;
        serde_json::from_str(&text).map_err(|error| {
            DomainError::InvalidData(format!("Invalid JSON resource '{relative_path}': {error}"))
        })
    }

    async fn copy_resource_to_file(
        &self,
        ...
        if let Some(parent) = destination.parent() {
            fs::create_dir_all(parent).await.map_err(|error| {
                DomainError::InternalError(format!("Failed to create directory: {error}"))
            })?;
        }

        let bytes = self.resources.read_bytes(resource_relative_path)?;
        fs::write(destination, bytes).await.map_err(|error| {
            DomainError::InternalError(format!("Failed to write resource file: {error}"))
        })
    }
...
impl ContentRepository for FileContentRepository {
    ...
    async fn get_content_index(&self) -> Result<Vec<ContentItem>, DomainError> {
        let index_items: Vec<ContentIndexItem> = self.read_resource_json("default/content/index.json")?;
