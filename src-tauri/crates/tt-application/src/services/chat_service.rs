...
impl ChatService {
    ...
    /// Raw JSONL bytes of a character chat payload.
    ///
    /// The server streams these to the browser and derives the revision token
    /// from the same bytes, so content and token always describe one revision.
