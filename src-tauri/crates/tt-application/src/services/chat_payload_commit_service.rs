...
use tt_ports::repositories::chat_payload_commit_repository::{
    ...
    ChatPayloadTarget, ChatPayloadVersion,
};
...
impl ChatPayloadCommitService {
    ...
    pub async fn begin(
        ...
        Ok(self
            .repository
            .begin(target_from_locator(locator), precondition)
            .await?)
    }

    /// Current stored revision, or `None` when the chat has no file yet.
    pub async fn read_version(
        &self,
        locator: ChatHistoryLocator,
    ) -> Result<Option<ChatPayloadVersion>, ApplicationError> {
        validate_chat_history_locator(&locator)?;
        Ok(self
            .repository
            .read_version(&target_from_locator(locator))
            .await?)
    }
    ...
    /// Publishes the staged payload and reports the revision now on disk.
    pub async fn finish(
        &self,
        session_id: &str,
        expected_size: u64,
        commit_reason: CurrentCommitReason,
    ) -> Result<(u64, ChatPayloadVersion), ApplicationError> {
